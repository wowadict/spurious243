...
#Region "Trainers"
    ...
    Public Sub On_CMSG_TRAINER_BUY_SPELL(ByRef packet As PacketClass, ByRef Client As ClientClass)
        ...
        WorldDatabase.Query(String.Format("SELECT * FROM trainer_spells WHERE entry = {0} AND learn_spell = {1};", WORLD_CREATUREs(cGUID).ID, SpellID), MySQLQuery)
    ...
    Public Sub SendTrainerList(ByRef c As CharacterObject, ByVal cGUID As ULong)
        ...
        WorldDatabase.Query(String.Format("SELECT * FROM npctext WHERE entry = {0};", noTrainID), npcTextSQLQuery)
        ...
        For Each SellRow As DataRow In SpellsList
            ...
            If c.HaveSpell(CType(SellRow.Item("learn_spell"), Integer)) Then
            ...
            packet.AddInt32(CType(SellRow.Item("is_prof"), Byte)) ' Same as above
...
#Region "Default Menu"
    ...
    Public Class TDefaultTalk
        ...
        Public Overrides Sub OnGossipHello(ByRef c As CharacterObject, ByVal cGUID As ULong)
            ...
            End If
            If (CREATURESDatabase(WORLD_CREATUREs(cGUID).ID).cNpcFlags And NPCFlags.UNIT_NPC_FLAG_ARENACHARTER) = NPCFlags.UNIT_NPC_FLAG_ARENACHARTER Then
