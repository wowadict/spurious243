    ...
    Public Class UpdateClass
        ...
        Public Sub AddToPacket(ByRef packet As PacketClass, ByVal updateType As ObjectUpdateType, ByRef updateObject As CharacterObject, Optional ByVal IsClientObject As Byte = 0)
            ...
            If updateType = ObjectUpdateType.UPDATETYPE_CREATE_OBJECT Or updateType = ObjectUpdateType.UPDATETYPE_CREATE_OBJECT_SELF Or updateType = ObjectUpdateType.UPDATETYPE_MOVEMENT Then
                Dim flags2 As Integer = updateObject.movementFlags
                ...
                If (flags2 And (MovementFlags.MOVEMENTFLAG_SWIMMING Or MovementFlags.MOVEMENTFLAG_FLYING2)) Then
                    packet.AddSingle(0)
                End If

                packet.AddInt32(0) 'Last fall time

                If (flags2 And MovementFlags.MOVEMENTFLAG_JUMPING) Then
                    packet.AddSingle(0) 'Unk
                    packet.AddSingle(0) 'SinAngle
                    packet.AddSingle(0) 'CosAngle
                    packet.AddSingle(0) 'XY Speed
                End If

                If (flags2 And MovementFlags.MOVEMENTFLAG_SPLINE) Then
                    packet.AddSingle(0) 'Unk1
                End If
        ...
        Public Sub AddToPacket(ByRef packet As PacketClass, ByVal updateType As ObjectUpdateType, ByRef updateObject As GameObjectObject, Optional ByVal IsClientObject As Byte = 0)
            ...
            If TypeOf packet Is UpdatePacketClass Then CType(packet, UpdatePacketClass).UpdatesCount += 1
        End Sub
        Public Sub AddToPacket(ByRef packet As PacketClass, ByVal updateType As ObjectUpdateType, ByRef updateObject As DynamicObjectObject, Optional ByVal IsClientObject As Byte = 0)
            packet.AddInt8(updateType)
            packet.AddPackGUID(updateObject.GUID)
...
#Region "Packets.ArrayBased"
    ...
    Public Class PacketClass
        ...
        Public Sub CompressUpdatePacket()
            ...
            Log.WriteLine(LogType.DEBUG, "COMPRESSED TIME {0} MS.", (timeGetTime - startTime))
        End Sub
