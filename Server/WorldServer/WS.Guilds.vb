...
#Region "WS.Guilds.Petition"
    ...
    Public Sub SendPetitionActivate(ByRef c As CharacterObject, ByVal cGUID As ULong)
        ...
        Dim Count As Byte = 3
        If WORLD_CREATUREs(cGUID).CreatureInfo.cNpcFlags And NPCFlags.UNIT_NPC_FLAG_TABARDVENDOR Then
            Count = 1
        End If
    ...
    Public Sub On_CMSG_PETITION_BUY(ByRef packet As PacketClass, ByRef Client As ClientClass)
        ...
        packet.GetInt32()
        If WORLD_CREATUREs.ContainsKey(GUID) = False OrElse (WORLD_CREATUREs(GUID).CreatureInfo.cNpcFlags And NPCFlags.UNIT_NPC_FLAG_ARENACHARTER) = 0 Then Exit Sub
        ...
        If (WORLD_CREATUREs(GUID).CreatureInfo.cNpcFlags And NPCFlags.UNIT_NPC_FLAG_TABARDVENDOR) Then
        ...
        Else
            Dim q As New DataTable
            WorldDatabase.Query(String.Format("SELECT arena_id FROM arena_teams WHERE arena_name = '{0}'", Name), q)
            If q.Rows.Count > 0 Then
