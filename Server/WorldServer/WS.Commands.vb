...
#Region "WS.Commands.Framework"
    ...
    Public Const HaileyGUID As ULong = Integer.MaxValue
    Public Const HaileyNAME As String = "Hailey"
...
#Region "WS.Commands.InternalCommands"
    ...
    Public Function cmdAIState(ByRef c As CharacterObject, ByVal Message As String) As Boolean
        ...
        Else
            With WORLD_CREATUREs(c.TargetGUID)
                c.CommandResponse(String.Format("Information for creature [{0}]:{1}ai = {2}{1}state = {3}", .Name, vbNewLine, .aiScript.ToString, .aiScript.State.ToString))
    ...
    Public Function cmdGetMax(ByRef c As CharacterObject, ByVal Message As String) As Boolean
        ...
        For Each row As DataRow In q.Rows
            ...
            Database.Query(String.Format("SELECT learn_spell FROM trainer_spells WHERE entry = {0}", row.Item("entry")), q2)
            For Each row2 As DataRow In q2.Rows
                If c.HaveSpell(CInt(row2.Item("learn_spell"))) = False Then c.LearnSpell(CInt(row2.Item("learn_spell")))
            Next
        Next
    ...
    Public Function cmdSetLevel(ByRef c As CharacterObject, ByVal tLevel As String) As Boolean
        ...
        If c.Level > Level Then
            c.CommandResponse("At the moment you can only add levels.")
            Return True
        End If

        For i As Short = c.Level To Level - 1
            TotalXP += XPTable(i)
        Next

        If CHARACTERs.ContainsKey(c.TargetGUID) Then
            CHARACTERs(c.TargetGUID).AddXP(TotalXP)
        Else
