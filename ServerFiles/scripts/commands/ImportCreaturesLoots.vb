		...
		Public Sub ParseLoot(ByVal Line As String)
			try
				...
				If Line.Substring(0, 5) = "loot=" Then
                    ...
                    MySQL.Update(String.Format("INSERT INTO loots (loot_item, loot_chance, loot_group) VALUES ({0}, {1} ,{2});", lootItem, Str(lootChance), lootID))
					loots_Count+=1
					Exit Sub
				End If
			catch e as exception
        ...
        Public Sub ParseCreature(ByVal Line As String)
			try
				...
				If line.IndexOf("loottemplate=") = 0 Then
                    ...
                    MySQL.Update(String.Format("UPDATE creatures SET creature_loot = {0} WHERE creature_id = {1};", lootID, creatureID))
					Creatures_Count+=1
					Exit Sub
				End If
			catch e as exception
